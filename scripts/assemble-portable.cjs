...
const exeName = exeCandidates.find((name) =>
...
if (exeName) {
  fs.copyFileSync(
    path.join(releaseDir, exeName),
    path.join(portableDir, exeName),
  );
} else {
  console.warn(
    "Executável Tauri não encontrado em src-tauri/target/release. Rode npm run tauri:build primeiro.",
  );
}

if (fs.existsSync(resourceSrc)) {
  fs.cpSync(resourceSrc, path.join(portableDir, "oracle-bridge"), {
    recursive: true,
  });
  // Também deixa o bridge ao lado do EXE em target/release para teste direto sem montar portable.
  fs.rmSync(path.join(releaseDir, "oracle-bridge"), {
    recursive: true,
    force: true,
  });
  fs.cpSync(resourceSrc, path.join(releaseDir, "oracle-bridge"), {
    recursive: true,
  });
}
