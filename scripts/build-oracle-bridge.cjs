...
const nodeRuntimeName =
  process.platform === "win32" ? "node-runtime.exe" : "node-runtime";
...
function ensureDependencies() {
  ...
  if (
    !fs.existsSync(typescriptBin) ||
    !fs.existsSync(viteBin) ||
    !fs.existsSync(tauriCli)
  ) {
...
if (!fs.existsSync(oracleDbSrc)) {
  throw new Error(
    "Dependência node_modules/oracledb não encontrada. Execute npm install antes do build.",
  );
}
