...
export function SqlTemplatePanel({ title, description, fields, defaults, templates }: Props) {
  ...
  const patchTemplateValues = useModuleStateStore((store) => store.patchTemplateValues);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      if (persistedState.startedAt) patchTemplateState(moduleKey, { elapsedMs: Date.now() - persistedState.startedAt });
    }, 100);
  ...
  async function runSql() {
    if (!statements.length) {
      patchTemplateState(moduleKey, { result: { ok: false, message: 'Nenhum comando SQL encontrado neste script.' } });
      return;
    }
    ...
    try {
      for (let i = 0; i < statements.length; i++) {
        if (useModuleStateStore.getState().templates[moduleKey]?.cancelRequested) {
          patchTemplateState(moduleKey, { result: {
            ok: false,
            message: 'Execução interrompida pelo usuário após o comando atual.',
      ...
      patchTemplateState(moduleKey, { busy: false, elapsedMs: finalElapsed, startedAt: undefined, cancelRequested: false });
    }
  }
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <label className="space-y-1 md:col-span-1">
            ...
            <select className="w-full rounded-xl bg-slate-950 border border-slate-800 px-3 py-2" value={templateId} onChange={(e) => patchTemplateState(moduleKey, { templateId: e.target.value })}>
          ...
          {fields.map((field) => (
            <label className="space-y-1" key={field.key}>
              ...
              {field.type === 'select' ? (
                <select className="w-full rounded-xl bg-slate-950 border border-slate-800 px-3 py-2" value={values[field.key] ?? ''} onChange={(e) => update(field.key, e.target.value)}>
                  {field.options?.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
                </select>
              ) : (
                <input className="w-full rounded-xl bg-slate-950 border border-slate-800 px-3 py-2" type={field.type ?? 'text'} min={field.min} max={field.max} step={field.step} value={values[field.key] ?? ''} onChange={(e) => update(field.key, e.target.value)} />
              )}
            </label>
          ))}
        </div>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-950 overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 bg-slate-900 px-4 py-3">
          <div>
            ...
            <p className="text-xs text-slate-500">Script gerado automaticamente</p>
          </div>

          <div className="mx-auto min-w-[280px] flex-1 max-w-xl">
            <div className={`rounded-xl border px-4 py-2 ${busy ? 'border-cyan-500/60 bg-cyan-950/30' : 'border-slate-800 bg-slate-950/70'}`}>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
                <span className="font-mono text-lg font-bold text-cyan-300">{formatElapsed(elapsedMs)}</span>
                {busy ? (
                  <span className="rounded-full bg-cyan-500/15 px-2 py-0.5 text-xs font-semibold text-cyan-200">Executando</span>
                ) : (
                  <span className="rounded-full bg-slate-800 px-2 py-0.5 text-xs font-semibold text-slate-300">Pronto</span>
                )}
                {current ? (
                  ...
                  <span className="text-slate-400">{statements.length} comando(s) no script</span>
                )}
              </div>
              {current && <div className="mt-1 truncate font-mono text-xs text-slate-400">{shortSql(current.statement)}</div>}
            </div>
          </div>

          <div className="flex gap-2">
            {busy && <button onClick={stopAfterCurrentStatement} className="inline-flex items-center gap-2 rounded-xl border border-rose-500/40 px-3 py-2 text-sm text-rose-200 hover:bg-rose-500/10">Parar</button>}
            <button onClick={copySql} className="inline-flex items-center gap-2 rounded-xl bg-slate-800 px-3 py-2 text-sm hover:bg-slate-700"><Copy size={16} /> Copiar</button>
            <button onClick={downloadSql} className="inline-flex items-center gap-2 rounded-xl bg-slate-800 px-3 py-2 text-sm hover:bg-slate-700"><Download size={16} /> Baixar</button>
            <button onClick={runSql} disabled={busy} className="inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-3 py-2 text-sm font-semibold text-slate-950 disabled:opacity-60"><Play size={16} /> {busy ? 'Aplicando...' : 'Aplicar no banco'}</button>
          </div>
        </div>
        <pre className="p-5 text-sm overflow-auto whitespace-pre-wrap text-slate-200">{sql}</pre>
      </section>

      {logs.length > 0 && (
        <section className="rounded-2xl border border-slate-800 bg-slate-950 overflow-hidden">
          <div className="border-b border-slate-800 bg-slate-900 px-4 py-3 font-semibold">Log de execução</div>
          <div className="max-h-64 overflow-auto">
            {logs.map((log) => (
              <div key={`${log.index}-${log.status}-${log.durationMs}`} className="border-b border-slate-800 px-4 py-3 last:border-b-0">
                <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
                  <span className="font-semibold text-slate-200">
                    Linha {log.line ?? '-'}{log.endLine && log.endLine !== log.line ? `-${log.endLine}` : ''} · Comando {log.index}
                  </span>
