...
export function ResultTable({ result }: { result?: QueryResult }) {
  ...
  if (!result.ok) {
    return (
      <pre className="mt-4 whitespace-pre-wrap rounded-2xl border border-rose-800 bg-rose-950/40 p-4 text-rose-200">
        {result.message}
      </pre>
    );
  }
      ...
      {rows.length > 0 && columns.length > 0 ? (
        <table className="min-w-full text-sm">
          <thead className="bg-slate-900 text-slate-300">
            <tr>
              {columns.map((c) => (
                <th className="px-3 py-2 text-left" key={c}>
                  {c}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={i} className="border-t border-slate-800">
                {columns.map((c) => (
                  <td className="whitespace-pre-wrap px-3 py-2" key={c}>
                    {String((row as any)[c] ?? "")}
                  </td>
                ))}
              </tr>
        ...
        <div className="p-4 text-slate-400">
          {result.message ||
            `Comando executado. Linhas afetadas: ${result.rowsAffected ?? 0}`}
        </div>
      )}
    </div>
  );
