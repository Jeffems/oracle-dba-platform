...
export function ScriptExecutionStatus({
  running,
  elapsedMs,
  current,
  logs,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-1 flex items-center gap-3">
            ...
            <span
              className={`rounded-full px-3 py-1 text-xs font-semibold ${running ? "bg-cyan-500/15 text-cyan-200" : "bg-slate-800 text-slate-300"}`}
            >
        ...
        {current && (
          <div className="min-w-[280px] rounded-xl border border-cyan-900/60 bg-cyan-950/20 px-4 py-3 text-sm">
            <div className="font-semibold text-cyan-200">
              Linha {current.line}
              {current.endLine !== current.line ? `-${current.endLine}` : ""} ·
              Comando {current.index}/{current.total}
            </div>
            <div className="mt-1 max-w-xl truncate font-mono text-xs text-slate-300">
