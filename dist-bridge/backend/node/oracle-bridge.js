...
function runSqlPlusAdminCommand(stmt, config) {
  return new Promise((resolve) => {
    ...
    const sqlplus = (0, node_child_process_1.spawn)(
      "sqlplus",
      ["-S", "/nolog"],
      { windowsHide: true },
    );
...
async function executeScript(config, sql, executionId) {
  ...
  const active = executionId
    ? { id: executionId, cancelled: false }
    : undefined;
  ...
  try {
    ...
    if (conn) await conn.close();
    if (executionId) activeExecutions.delete(executionId);
  }
}
...
function runPatchProcess(config) {
  return new Promise((resolve) => {
    try {
      ...
      const tempBat = node_path_1.default.join(
        node_os_1.default.tmpdir(),
        `oracle_patch_${Date.now()}.bat`,
      );
      ...
      const child = (0, node_child_process_1.spawn)(
        ...
        ["/c", tempBat],
        {
          windowsHide: true,
          cwd: String(
            config.workRoot || config.patchDir || node_os_1.default.tmpdir(),
          ),
        },
      );
      let output = "",
...
async function runPatchProcessStream(config, res) {
  ...
  try {
    ...
    const flushLogFile = () => {
      try {
        if (!node_fs_1.default.existsSync(logPath)) return;
...
function persistAgentSnapshot(snapshot) {
  node_fs_1.default.appendFileSync(
    agentMetricsFile(),
...
function readRecentAgentMetrics(limit = 20) {
  ...
  if (!node_fs_1.default.existsSync(file)) return [];
